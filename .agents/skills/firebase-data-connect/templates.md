...
mutation UpdateNote($id: UUID!, $title: String, $content: String) @auth(level: USER) {
	note_update(first: { where: { id: { eq: $id }, owner: { uid: { eq_expr: "auth.uid" } } } }, data: { title: $title, content: $content })
}
...
@auth(level: PUBLIC)
@refresh(onMutationExecuted: { operation: "CreateItem", condition: "request.variables.categoryId == mutation.variables.categoryId" }) {

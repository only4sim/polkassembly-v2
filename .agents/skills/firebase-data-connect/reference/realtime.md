...
query ChatRoom($roomId: UUID!) @auth(level: PUBLIC) @refresh(onMutationExecuted: { operation: "SendMessage", condition: "mutation.variables.roomId == request.variables.roomId" }) {
...
@auth(level: USER)
@refresh(onMutationExecuted: { operation: "UpdateOrderStatus", condition: "request.variables.userId == mutation.variables.userId" })
